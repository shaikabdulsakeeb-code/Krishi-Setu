...
export default function FarmerDashboard() {
  ...
  return <div className="space-y-8">
    <section className="rounded-2xl bg-gradient-to-br from-green-800 to-green-600 px-6 py-8 text-white shadow-sm sm:px-8">
      ...
      <h1 className="mt-2 text-3xl font-bold">Namaste, {userData?.name || 'farmer'}.</h1>
    ...
    <section className="grid gap-4 sm:grid-cols-3">
      {stats.map(({ label, value, icon: Icon, colour }) => <div key={label} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div className={`flex h-10 w-10 items-center justify-center rounded-lg ${colour}`}><Icon className="h-5 w-5" /></div><p className="mt-4 text-3xl font-bold text-gray-900">{value}</p><p className="mt-1 text-sm text-gray-500">{label}</p></div>)}
    </section>
