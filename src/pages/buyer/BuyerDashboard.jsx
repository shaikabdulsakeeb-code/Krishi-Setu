...
export default function BuyerDashboard() {
  ...
  return <div className="space-y-8">
    <section className="rounded-2xl bg-gradient-to-br from-emerald-800 to-green-600 px-6 py-8 text-white shadow-sm sm:px-8"><p className="text-sm font-semibold tracking-wide text-green-100">BUYER DASHBOARD</p><h1 className="mt-2 text-3xl font-bold">Welcome, {userData?.name || 'buyer'}.</h1><p className="mt-2 max-w-2xl text-green-50">Buy directly from farmers with clear quantities, delivery dates, and transport estimates.</p><Link to="/buyer/browse" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-green-800 shadow-sm transition hover:bg-green-50"><Search className="h-4 w-4" /> Browse crop market</Link></section>
    <section className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-green-100 text-green-700"><Sprout className="h-5 w-5" /></div><p className="mt-4 text-3xl font-bold text-gray-900">{cropCount}</p><p className="mt-1 text-sm text-gray-500">Crops in the market</p></div><div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-100 text-blue-700"><Handshake className="h-5 w-5" /></div><p className="mt-4 text-3xl font-bold text-gray-900">{activeDeals}</p><p className="mt-1 text-sm text-gray-500">Active offers and deals</p></div></section>
